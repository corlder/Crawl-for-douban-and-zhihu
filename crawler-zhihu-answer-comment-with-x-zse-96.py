...
def get_zhihu_answer_comments(answer_id, limit=20,next_url=None):
    ...
    :return: 评论列表
    """
    url = f"https://www.zhihu.com/api/v4/answers/{answer_id}/root_comments?order=score&limit={limit}&status=open"
...
def get_zhihu_answer_child_comments(root_comment_id, limit=20, next_url=None):
    ...
    url = f"https://www.zhihu.com/api/v4/comment_v5/comment/{root_comment_id}/child_comment?limit={limit}&order_by=ts"
    ...
    try:
        ...
        print(f"请求失败: {e}")
        if requests.code == 403:
            print("请求过于频繁，等待一段时间后重试...")
            time.sleep(60)
        elif requests.code == 404:
            print("评论不存在或已被删除")
        elif requests.code == 500:
            print("服务器错误，请稍后再试")
        else:       
            print("发生其他错误")
        return None
